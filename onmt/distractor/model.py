...
class DGModel(nn.Module):
    ...
    def forward(self, src, ques, ans, adj, tgt,
                sent_lengths, word_lengths, ques_length, ans_length,
                dec_state=None):

        tgt = tgt[:-1]  # exclude last target from inputs

        word_mem_bank, sent_mem_bank, quesinit, C = self.encoder(
            src, ques, ans, adj, sent_lengths, word_lengths, ques_length, ans_length)
