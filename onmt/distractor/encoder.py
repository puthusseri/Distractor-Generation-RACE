...
class DistractorEncoder(nn.Module):
    def __init__(self, rnn_type,
        ...
        # ggnn
        self.ggnn = nn.GRU(hidden_size, hidden_size)
    ...
    def context_attention(self, c, q):
        m = nn.MaxPool1d(q.size(1))
        M = torch.bmm(self.linear(c), self.linear(q).transpose(1,2)) 
        a = F.softmax(m(M).squeeze(-1), -1).unsqueeze(-1).expand(-1, -1, c.size(2))
        C = c * a
        return C
    ...
    def forward(self, src, ques, ans, adj, sent_length, word_length, 
                ques_length, ans_length):
        ...
        # question
        wrapped_ques = PermutationWrapper(ques, ques_length, 
                               rnn_type=self.rnn_type)
        sorted_ques, sorted_ques_length = wrapped_ques.sort()
        sorted_ques_emb = self.embeddings(sorted_ques.unsqueeze(-1))
        sorted_ques_bank, sorted_ques_state, _ = \
            self.word_encoder(sorted_ques_emb, sorted_ques_length)  # encode
        ques_bank, ques_state = \
            wrapped_ques.remap(sorted_ques_bank, sorted_ques_state)  # remap
        ...
        return word_bank, sent_bank, quesinit_state, None
