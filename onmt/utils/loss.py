...
class S2SLossCompute(nn.Module):
    ...
    def get_bleu_batch(self, out, dec_out, n=None):
        out = out.tolist()
        dec_out = dec_out.tolist()
        bleus = []
        if n is None:
            for i in range(len(out)):
                bleus.append(self.get_bleu(out[i], dec_out[i]))
            return bleus
        else:
            for i in range(len(out)):
                bleus.append(self.get_bleu(out[i], dec_out[i], n))
            return bleus
    ...
    def compute_loss(self, batch, output, attns, normalization):
        ...
        self.cnt += 1
        if self.cnt % 10000 == 9999 or self.flag:
            if self.flag is False:
                self.bigger_cnt, self.smaller_cnt = 0, 0
                self.flag = True
            probs = F.softmax(self.generator[0](output), -1)
            tf_action = torch.argmax(probs, 2)
            m = Categorical(probs)
            action = m.sample()
            reward = m.log_prob(action)
            sample_bleu = self.get_bleu_batch(target.transpose(0,1),
                                          action.transpose(0,1))
            tf_bleu = self.get_bleu_batch(target.transpose(0,1),
                                      tf_action.transpose(0,1))
            for i in range(len(sample_bleu)):
                if sample_bleu[i] > tf_bleu[i] and sample_bleu[i] > 0.0001:
                    #print(self.cnt, "bigger", i, sample_bleu[i], tf_bleu[i])
                    self.bigger_cnt += 1
                if sample_bleu[i] < tf_bleu[i] and tf_bleu[i] > 0.0001:
                    #print(self.cnt, "smaller", i, sample_bleu[i], tf_bleu[i])
                    self.smaller_cnt += 1
                    if self.smaller_cnt == 500:
                        print("cnt:", self.bigger_cnt, self.smaller_cnt)
                        print("ratio:", self.bigger_cnt / (self.smaller_cnt + self.bigger_cnt))
                        self.flag = False
                        break
    ...
    def monolithic_compute_loss(self, batch, output, attns):
        ...
        tf_action = torch.argmax(probs, 2)
        tf_bleu = self.get_bleu_batch(target.transpose(0,1),
                                      tf_action.transpose(0,1), 4)
        bottled_output = self._bottle(output)
        scores = self.generator(bottled_output)
        gtruth = target.view(-1)
        loss = self.criterion(scores, gtruth)
        stats = self._stats(loss.clone(), scores, gtruth)
        return stats, tf_bleu
