...
class TextDataset(DatasetBase):
    ...
    @staticmethod
    def make_text_examples_nfeats_tpl(text_iter, text_path, truncate, tfidf):
        ...
        examples_nfeats_iter = \
            TextDataset.make_examples(text_iter, truncate, tfidf)
    ...
    @staticmethod
    def make_examples(text_iter, truncate, tfidf):
        for i, line in enumerate(text_iter):
            ex = json.loads(line)

            src_words, adj_matrix, src_feats, src_n_feats = \
                TextDataset.extract_text_features(ex['sents'], 
                                        side='src', tfidf=tfidf)
            ans_words, ans_feats, ans_n_feats = \
                TextDataset.extract_text_features(ex['answer'])
            qu_words, qu_feats, qu_n_feats = \
                TextDataset.extract_text_features(ex['question'])
            if len(qu_words) == 0: continue
            ...
            example_dict = {
                ...
                'answer': ans_words,
                'question': qu_words,
                'ques_ans': tuple(list(qu_words) + list(ans_words)),
                'tgt': tgt_words,
                "indices": i,
                'id': ex['id'],
    ...
    def get_fields(data_type):
        ...
        fields["adj"] = torchtext.data.NestedField(
            torchtext.data.Field(sequential=True, use_vocab=False,
                                 dtype=torch.float32, pad_token=0), 
